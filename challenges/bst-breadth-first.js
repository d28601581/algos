...
const minimumDistance = grid => {
  ...
  while(queue.length > 0){
    ...
    memory.add([x,y].toString());

    if(x < 0 || x >= grid.length || y >= grid[0].length || y < 0) continue;
    const curr = grid[x][y];

    if(curr === 2) return count;
    else if (curr === 1) continue;
    else if(curr === 0) {
      if(grid.length > x && grid[0].length > y) {
        if(!memory.has([x + 1, y].toString())) queue.push([count + 1, x + 1, y]);
        if(!memory.has([x, y + 1].toString())) queue.push([count + 1, x, y + 1]);
        if(!memory.has([x - 1, y].toString())) queue.push([count + 1, x - 1, y]);
        if(!memory.has([x, y - 1].toString())) queue.push([count + 1, x, y - 1]);
      }
    } 
...
//   [1, 0, 0, 0]
// ];
//console.log(minimumDistance(grid2))
