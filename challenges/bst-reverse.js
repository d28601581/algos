...
const sortedArrayToBST = (arr, left = 0, right = arr.length - 1) => {
  ...
  if(left === right) {
    return new BinarySearchTree(arr[left]);
  }
  let mid = left + Math.ceil((right - left) / 2);
  let tree = new BinarySearchTree(arr[mid]);
  tree.left = sortedArrayToBST(arr, left, mid - 1);
  tree.right = sortedArrayToBST(arr, mid + 1, right);
  return tree;
};
