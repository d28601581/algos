...
Use an object store for the cache, as the function p arameter may be any value
...
const cachePromiseFunction = get => {
  const cache = {};
  return (str) => {
    if(!cache[str]) cache[str] = Promise.resolve(get(str))
    return cache[str];
  }
};
/*
